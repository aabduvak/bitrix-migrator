...
def check_product(data: dict) -> bool:
    params = {
        "select": ["id", "iblockId", "*"],
        "filter": {"iblockId": IBLOCK_ID, "code": str(data.get("code"))},
        "order": {"id": "ASC"},
    }
...
def set_price(price: int, product_id: int, currency: str = "UZS"):
    params = dict(
        fields=dict(
            catalogGroupId=1, currency=currency, price=price, productId=product_id
        )
    )
...
def create_product(data: dict):
    ...
    if "property" in data:
        params["fields"][data["property"]["key"]] = {"value": data["property"]["value"]}
