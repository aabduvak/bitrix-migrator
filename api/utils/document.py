...
def set_supplier(id: int, supplier_id: int = 1, type_id: int = 4):
    params = dict(
        fields=dict(
            documentId=id, entityTypeId=type_id, entityId=supplier_id  # Company type
        )
    )
...
def update_document_total(id: int):
    ...
    params = dict(id=id, fields=dict(total=str(total)))

    response = BITRIX.call("catalog.document.update", params)
    return response


def add_item_document(
    document: Document, product: Product, data: dict
) -> DocumentProduct:
    params = dict(
        fields=dict(
